...
happyExpList = Happy_Data_Array.listArray (0,38) ([3008,384,32,0,512,0,0,0,0,0,0,0,0,32,0,0,4,0,0,48128,6144,3008,384,0,49152,32779,1,0,0
	])

{-# NOINLINE happyExpListPerState #-}
happyExpListPerState st =
    token_strs_expected
  where token_strs = ["error","%dummy","%start_parseCalc","Expr","BOOL","INT","int","TRUE","FALSE","PRINT","WHERE","UNION","PRED","SUB","OBJ","AND","OR","FROM","NOT","ADD","DELETE","CHANGE","RESTRICT","SORT","GET","'='","','","%eof"]
...
happyNewToken action sts stk (tk:tks) =
	...
	case tk of {
	TokenBoolType p -> cont 5;
	TokenIntType p -> cont 6;
	TokenInt p s -> cont 7;
	TokenTrue p -> cont 8;
	TokenFalse p -> cont 9;
	TokenPrint p -> cont 10;
	TokenWhere p -> cont 11;
	TokenUnion p -> cont 12;
	TokenPred p -> cont 13;
	TokenSub p -> cont 14;
	TokenObj p -> cont 15;
	TokenAnd p -> cont 16;
	...
	TokenNot p -> cont 19;
	TokenAdd p -> cont 20;
	TokenDelete p -> cont 21;
	TokenChange p -> cont 22;
	TokenRestrict p -> cont 23;
	TokenSort p -> cont 24;
	TokenGet p -> cont 25;
	TokenEquals p -> cont 26;
	TokenComma p -> cont 27;
	_ -> happyError' ((tk:tks), [])
	}
...
data Expr= Print Expr | Union Expr Expr | Filter Expr Comp | OutputAll Expr | Comp Expr Expr | And Cond Cond | Not Cond |Or Cond |
